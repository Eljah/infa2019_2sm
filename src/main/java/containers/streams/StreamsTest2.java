...
//                }).
        ...
        System.out.println((Stream.generate(() -> 1d).parallel().limit(10).map(i -> i.toString()).reduce(0, (total, word) -> {
                    System.out.println("Total " + total + "; word " + word);
                    return total + word.length();
                },
                (total1, total2) -> {
        ...
        Supplier supplier=()->{return "empty as on "+new Date().toString();};
        String sum3="aa"+string2.orElseGet(()->{return "empty as on "+new Date().toString();})+string0.orElseGet(supplier)+string1.orElseGet(supplier);
        ...
        optionalStream.peek((str)->{
            System.out.println("Put to StringBuilder "+str);str.ifPresent(stringBuilder::append);}).count();
